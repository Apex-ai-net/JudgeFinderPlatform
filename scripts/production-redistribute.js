...
function logOperation(operation, data) {
  const redactValue = (value) => {
    ...
    if (typeof value === 'string') {
      if (/(password|secret|token|key|apikey)/i.test(value)) {
        return '[REDACTED]'
      }
      return value
    }
    if (typeof value === 'object') {
      ...
      for (const [k, v] of Object.entries(value)) {
        redactedObj[k] = /(password|secret|token|key|apikey)/i.test(k)
          ? '[REDACTED]'
          : redactValue(v)
      }
      return redactedObj
  ...
  console.log(`📝 ${operation}:`, JSON.stringify(redactedData, null, 2))
}
...
async function confirmProductionOperation() {
  ...
  const answer2 = await askQuestion(
    '❓ Are you sure you want to proceed with PRODUCTION redistribution? (type "REDISTRIBUTE" to confirm): '
  )
...
async function redistributeProductionCases(judges, totalCases) {
  ...
  const progressTimer = setInterval(() => {
    ...
    console.log(
      `⏳ Progress: ${session.stats.redistributed.toLocaleString()}/${totalCases.toLocaleString()} cases (${((session.stats.redistributed / totalCases) * 100).toFixed(2)}%) - ETA: ${Math.round(remaining / 60)} minutes`
    )
...
async function verifyProductionRedistribution() {
  ...
  try {
    ...
    console.log(
      `  Linked cases: ${linkedCases?.toLocaleString()} (${verification.linkagePercentage}%)`
    )
...
function generateProductionReport() {
  ...
  console.log('\n⏱️  PERFORMANCE:')
  console.log(
    `  Total time: ${Math.round(elapsed / 60)} minutes ${Math.round(elapsed % 60)} seconds`
  )
  console.log(
    `  Processing rate: ${Math.round(session.stats.redistributed / elapsed)} cases/second`
  )
