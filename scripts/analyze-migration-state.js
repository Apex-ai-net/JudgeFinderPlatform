...
async function analyzeMigrationState() {
  ...
  log(`  Pending migrations: ${localMigrations.length - (appliedMigrations?.length || 0)}`, 'bright');
  ...
  const partiallyApplied = migrationAnalysis.filter(m => !m.isApplied && m.someTablesExist && !m.allTablesExist);
  ...
  if (appliedButNotTracked.length > 0) {
    ...
    console.log('BEGIN;');
    appliedButNotTracked.forEach(m => {
      console.log(`INSERT INTO supabase_migrations.schema_migrations (version) VALUES ('${m.timestamp}');`);
    });
    console.log('COMMIT;');
  }
