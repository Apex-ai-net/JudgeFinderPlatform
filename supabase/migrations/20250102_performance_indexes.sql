...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_judges_court_appointed
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_judges_appointed_date
ON judges(appointed_date DESC NULLS LAST);

-- Slug lookup for SEO-friendly URLs
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_judges_slug
ON judges(slug)
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_cases_judge_decision
ON cases(judge_id, decision_date DESC)
WHERE decision_date IS NOT NULL;

-- Case type analysis for bias detection
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_cases_judge_type
ON cases(judge_id, case_type);

-- Outcome analysis for judicial analytics
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_cases_judge_outcome
ON cases(judge_id, outcome)
WHERE outcome IS NOT NULL;

-- Filing date range queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_cases_filing_date
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_courts_slug
ON courts(slug)
WHERE slug IS NOT NULL;

-- Court level filtering (Superior, Supreme, etc.)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_courts_level
...
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'decisions') THEN
    ...
    CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_decisions_judge
    ON decisions(judge_id, date_filed DESC);

    -- Case-specific decisions
    CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_decisions_case
    ON decisions(case_id, date_filed DESC);

    -- Full-text search on decision text
    CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_decisions_text_search
...
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'judge_analytics') THEN
    ...
    CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_judge_analytics_judge
    ON judge_analytics(judge_id, generated_at DESC);

    -- Stale analytics cleanup
    CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_judge_analytics_generated
...
--
-- Index creation runs CONCURRENTLY to avoid locking tables
-- Safe to run on production database without downtime
--
-- Monitor index usage:
