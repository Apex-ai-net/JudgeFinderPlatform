...
COMMENT ON POLICY "judge_court_positions_public_select" ON public.judge_court_positions IS
  'Anyone can view judge court positions';
COMMENT ON POLICY "judge_court_positions_service_all" ON public.judge_court_positions IS
  'Service role and service account have full access';
COMMENT ON POLICY "judge_court_positions_admin_write" ON public.judge_court_positions IS
  'Admins can manage judge court positions';
...
COMMENT ON POLICY "sync_queue_service_all" ON public.sync_queue IS
  'Service role and service account have full access to sync queue';
COMMENT ON POLICY "sync_queue_admin_all" ON public.sync_queue IS
  'Admins can view and manage sync queue';
...
-- Users can view issues related to their own reports
...
CREATE POLICY "profile_issues_user_select"
  ...
  USING (
    ...
    EXISTS (
      ...
      AND clerk_user_id = profile_issues.reporter_id
    )
  );

COMMENT ON POLICY "profile_issues_service_all" ON public.profile_issues IS
  'Service role and service account have full access';
COMMENT ON POLICY "profile_issues_admin_all" ON public.profile_issues IS
  'Admins can manage all profile issues';
COMMENT ON POLICY "profile_issues_user_select" ON public.profile_issues IS
...
COMMENT ON POLICY "ad_waitlist_service_all" ON public.ad_waitlist IS
  'Service role and service account have full access';
COMMENT ON POLICY "ad_waitlist_user_insert" ON public.ad_waitlist IS
  'Authenticated users can add themselves to ad waitlist';
COMMENT ON POLICY "ad_waitlist_user_select" ON public.ad_waitlist IS
  'Users can view their own waitlist entries';
COMMENT ON POLICY "ad_waitlist_admin_all" ON public.ad_waitlist IS
...
COMMENT ON POLICY "ad_events_service_all" ON public.ad_events IS
  'Service role and service account have full access';
COMMENT ON POLICY "ad_events_public_insert" ON public.ad_events IS
  'Anyone can log ad impressions and clicks for analytics';
COMMENT ON POLICY "ad_events_admin_select" ON public.ad_events IS
  'Admins can view all ad events';
COMMENT ON POLICY "ad_events_owner_select" ON public.ad_events IS
  'Ad owners can view events for their own ads';
...
COMMENT ON POLICY "user_push_tokens_service_all" ON public.user_push_tokens IS
  'Service role and service account have full access';
COMMENT ON POLICY "user_push_tokens_user_all" ON public.user_push_tokens IS
  'Users can manage their own push notification tokens';
COMMENT ON POLICY "user_push_tokens_admin_select" ON public.user_push_tokens IS
...
-- Authenticated users can create ad spots
DROP POLICY IF EXISTS "ad_spots_user_insert" ON public.ad_spots;
...
-- Users can update their own ad spots
DROP POLICY IF EXISTS "ad_spots_user_update" ON public.ad_spots;
...
-- Users can delete their own ad spots
DROP POLICY IF EXISTS "ad_spots_user_delete" ON public.ad_spots;
...
COMMENT ON POLICY "ad_spots_user_insert" ON public.ad_spots IS
  'Authenticated users can create their own ad spots';
COMMENT ON POLICY "ad_spots_user_update" ON public.ad_spots IS
  'Users can update their own ad spots';
COMMENT ON POLICY "ad_spots_user_delete" ON public.ad_spots IS
  'Users can delete their own ad spots';
