 ...
 *
 * COST PROTECTION:
 * - Budget limits: $50/day, $500/month, $0.10/request max
 * - Cost estimation before AI calls
 * - Automatic tracking and budget enforcement
 */

const { GoogleGenerativeAI } = require('@google/generative-ai')

// Import cost tracker - will be required dynamically to avoid ESM issues
let getCostTracker = null
let BudgetExceededError = null

async function initializeCostTracker() {
  if (!getCostTracker) {
    try {
      const costTrackerModule = await import('../ai/cost-tracker.js')
      getCostTracker = costTrackerModule.getCostTracker
      BudgetExceededError = costTrackerModule.BudgetExceededError
    } catch (error) {
...
async function generateJudicialAnalytics(judge, caseDocuments) {
  ...
  try {
    ...
    // COST PROTECTION: Estimate cost before making AI request
    const estimatedCost = costTracker.estimateAnalyticsCost(documentSummaries.length, modelName)
    console.log(`💰 Estimated cost for ${judge.name}: $${estimatedCost.toFixed(4)} (${documentSummaries.length} cases)`)

    // COST PROTECTION: Check budget limits
    const budgetStatus = await costTracker.checkBudget(estimatedCost)

    if (!budgetStatus.canProceed) {
      console.error(`🚫 AI Budget limit reached for ${judge.name}: ${budgetStatus.message}`)
      if (BudgetExceededError) {
        throw new BudgetExceededError(budgetStatus)
      } else {
        throw new Error(budgetStatus.message || 'AI budget limit exceeded')
      }
    }

    // Warn if approaching limits
    if (budgetStatus.warningLevel === 'warning') {
      console.warn(`⚠️  ${budgetStatus.message}`)
    }
    ...
    // COST PROTECTION: Record actual cost after successful generation
    const actualCost = costTracker.calculateActualCost(inputTokens, outputTokens, modelName)
    await costTracker.recordCost(actualCost, {
      judgeId: judge.id,
      judgeName: judge.name,
      model: modelName,
      inputTokens,
      outputTokens,
      caseCount: documentSummaries.length,
      endpoint: 'generateJudicialAnalytics',
    ...
    if (error.name === 'BudgetExceededError') {
      // Return analytics with budget exceeded flag instead of failing completely
      return {
        ...generateFallbackAnalytics(judge, 'budget_exceeded', error.message),
        budget_status: error.budgetStatus
      }
    }
...
async function generateAnalyticsWithOpenAI(judge, caseDocuments) {
  ...
  try {
    ...
    const documentSummaries = caseDocuments
      .filter(doc => doc.analyzable && doc.plain_text)
      .slice(0, 30) // Smaller limit for OpenAI (more expensive)
      .map(doc => ({
        case_name: doc.case_name,
        category: doc.case_category,
        outcome: doc.case_outcome,
    ...
    // COST PROTECTION: Estimate and check budget for OpenAI (more expensive)
    const estimatedCost = costTracker.estimateAnalyticsCost(documentSummaries.length, modelName)
    console.log(`💰 Estimated OpenAI cost for ${judge.name}: $${estimatedCost.toFixed(4)}`)

    const budgetStatus = await costTracker.checkBudget(estimatedCost)
    if (!budgetStatus.canProceed) {
      console.error(`🚫 OpenAI budget limit reached: ${budgetStatus.message}`)
      return generateFallbackAnalytics(judge, 'budget_exceeded', budgetStatus.message)
    }
...
Base confidence on case quantity and quality.`
    ...
    // COST PROTECTION: Record OpenAI cost
    const actualCost = costTracker.calculateActualCost(inputTokens, outputTokens, modelName)
    await costTracker.recordCost(actualCost, {
      judgeId: judge.id,
      judgeName: judge.name,
      ...
      outputTokens,
      caseCount: documentSummaries.length,
      endpoint: 'generateAnalyticsWithOpenAI',
      timestamp: new Date().toISOString()
    })
