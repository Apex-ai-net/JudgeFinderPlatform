...
const ENV_VARIABLES: EnvVariable[] = [
  ...
  },
  {
    ...
    required: true,
    description: 'Supabase JWT secret for signing service account tokens',
  ...
  {
    name: 'ENCRYPTION_KEY',
    required: false, // Required in production, optional in development
    description: 'Encryption key for sensitive data and service account operations',
    validator: (value) => value.length >= 32,
  },
...
export function validateEnvironment(): EnvValidationResult {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    // JWT secret is required for service account
    if (!process.env.SUPABASE_JWT_SECRET) {
      errors.push('SUPABASE_JWT_SECRET is required in production for service account operations')
    } else if (process.env.SUPABASE_JWT_SECRET.length < 32) {
      errors.push('SUPABASE_JWT_SECRET must be at least 32 characters long')
    }
  }
  ...
  if (process.env.NODE_ENV === 'development') {
    if (!process.env.ENCRYPTION_KEY) {
      warnings.push('ENCRYPTION_KEY not set. Some service account features may not work.')
    }

    if (!process.env.SUPABASE_JWT_SECRET) {
      warnings.push('SUPABASE_JWT_SECRET not set. Service account client will not work.')
    }
  }
...
export function isProductionReady(): boolean {
  ...
  const missingProduction = productionRequirements.filter((name) => !process.env[name])
  ...
  const missingRecommended = recommendedProduction.filter((name) => !process.env[name])
