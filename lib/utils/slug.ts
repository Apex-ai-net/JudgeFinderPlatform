 ...
 */
export function generateSlug(name: string, options: SlugGenerationOptions = {}): string {
...
export function parseJudgeName(name: string): {
  ...
  const lastName =
    namePartsWithoutSuffix.length > 1
      ? namePartsWithoutSuffix[namePartsWithoutSuffix.length - 1]
      : ''
  const middleName =
    namePartsWithoutSuffix.length > 2
...
export function generateJudgeUrlVariations(judgeName: string, baseUrl: string = ''): string[] {
  ...
  const topVariations = slugVariations.filter((slug) => slug !== canonical).slice(0, 3) // Limit to prevent sitemap bloat
...
export function generateCourtSlugVariations(name: string): string[] {
  ...
  const shortWords = words.filter(
    (word) => !['of', 'the', 'in', 'and', 'for'].includes(word) && word.length > 2
  )
