...
const panelClass =
  'rounded-2xl border border-border bg-[hsl(var(--bg-2))] p-5 shadow-[0_1px_0_rgba(38,43,54,0.35)]'
const selectClass =
...
const RISK_BADGES = {
  ...
  Medium:
    'bg-[rgba(251,211,141,0.18)] text-[color:hsl(var(--warn))] border border-[rgba(251,211,141,0.35)]',
...
export function BiasAnalyticsDashboard(): JSX.Element {
  ...
  if (!analyticsData) {
    return (
      <section className={cn(panelClass, 'text-center text-sm text-[color:hsl(var(--text-2))]')}>
        <AlertTriangle
          className="mx-auto mb-3 h-10 w-10 text-[color:hsl(var(--warn))]"
          aria-hidden
        />
  ...
  return (
    <section className="space-y-6">
      <header className={cn(panelClass, 'space-y-4')} aria-label="Dashboard filters">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            <div className="text-xs uppercase tracking-[0.28em] text-[color:hsl(var(--text-3))]">
              Platform analytics
            </div>
            ...
            <p className="text-sm text-[color:hsl(var(--text-2))]">
              Aggregated from {analyticsData.overview.total_judges.toLocaleString()} judges ·{' '}
              {analyticsData.overview.cases_analyzed.toLocaleString()} cases processed
            </p>
          </div>
      ...
      <nav
        ...
        role="tablist"
        aria-label="Bias analytics sections"
      >
      ...
      {activeTab === 'overview' && (
        ...
        >
          ...
          <article className={panelClass}>
            <h3 className="text-lg font-semibold text-[color:hsl(var(--text-1))]">
              Settlement patterns by case type
            </h3>
            <ResponsiveContainer width="100%" height={280}>
      ...
      {activeTab === 'judges' && (
        <section id="judges-panel" role="tabpanel" aria-labelledby="judges" className="space-y-5">
          <div
            className={cn(
              panelClass,
              'flex flex-wrap items-center gap-3 text-sm text-[color:hsl(var(--text-2))]'
            )}
          >
                ...
                <tbody className="divide-y divide-border/60">
                  {analyticsData.bias_indicators.map((judge) => (
                    <tr key={judge.judge_id} className="hover:bg-[hsl(var(--bg-1))]/60">
                      <td className="px-6 py-4 text-[color:hsl(var(--text-1))]">
                        {judge.judge_name}
                      </td>
                      <td className="px-6 py-4 text-[color:hsl(var(--text-2))]">
                      ...
                      <td className="px-6 py-4">
                        <Badge
                          className={cn(
                            ...
                            RISK_BADGES[judge.bias_risk_level]
                          )}
                        >
      ...
      {activeTab === 'geographic' && (
        <section id="geographic-panel" role="tabpanel" aria-labelledby="geographic">
          <article className={panelClass}>
            <h3 className="text-lg font-semibold text-[color:hsl(var(--text-1))]">
              Geographic distribution of bias patterns
            </h3>
                ...
                <tbody className="divide-y divide-border/60">
                  {analyticsData.geographic_distribution.map((item) => (
                    <tr key={item.jurisdiction} className="hover:bg-[hsl(var(--bg-1))]/60">
                      <td className="px-4 py-3 text-[color:hsl(var(--text-1))]">
                        {item.jurisdiction}
                      </td>
                      <td className="px-4 py-3 text-[color:hsl(var(--text-2))]">
                        {item.judge_count}
                      </td>
                      <td className="px-4 py-3 text-[color:hsl(var(--text-2))]">
                        {item.avg_consistency.toFixed(1)}
                      </td>
                      <td className="px-4 py-3 text-[color:hsl(var(--text-2))]">
                        {(item.avg_settlement_rate * 100).toFixed(1)}%
                      </td>
                    </tr>
                  ))}
                </tbody>
    ...
    <article className={panelClass}>
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div
            className={cn(
              'mt-3 inline-flex rounded-full px-4 py-2 text-lg font-semibold',
              toneClass
            )}
          >
            {value}
