...
> = {
  HIGH: {
    label: 'High quality',
    description:
      'Large sample size, clean source documents, and agreement across primary and fallback models.',
    badgeClass:
  ...
  LOW: {
    label: 'Low quality',
    description:
      'Insufficient sample size or conflicting data sources; analytics hidden or limited.',
    badgeClass:
      'border border-[rgba(251,211,141,0.4)] bg-[rgba(251,211,141,0.18)] text-[color:hsl(var(--warn))]',
  },
}

export function QualityBadge({ level, explanation, className }: QualityBadgeProps): JSX.Element {
  ...
  return (
    <span className={cn('inline-flex items-center gap-1.5 text-xs font-medium', className)}>
      <span
        className={cn('inline-flex items-center gap-1.5 rounded-full px-3 py-1', meta.badgeClass)}
      >
        {meta.label}
      </span>
      <InfoTooltip
