...
export function CaseOutcomeStatistics({ judge }: CaseOutcomeStatisticsProps): JSX.Element {
  ...
  const isSeriesActive = useCallback(
    (dataKey: string) => activeSeries.includes(dataKey),
    [activeSeries]
  )
  ...
  if (!outcomeStats) {
    return (
      <section className={cn(containerClass, 'text-center text-sm text-muted-foreground')}>
        ...
        <p>
          Case outcome statistics are unavailable right now. Refresh the page or try narrowing your
          filters.
        </p>
      </section>
    )
    ...
    return (
      <section className={cn(containerClass, 'text-sm text-muted-foreground')}>
        ...
        <p className="leading-6">
          We need at least {MIN_SAMPLE_SIZE} recent decisions with outcome classifications before
          rendering trend charts. Once the sync backlog clears, this view will repopulate
          automatically.
        </p>
      </section>
    )
    ...
    <section className={containerClass}>
      ...
      {activeView === 'breakdown' && (
        <section id="breakdown-panel" role="tabpanel" aria-labelledby="breakdown">
          ...
          <div
            className="rounded-2xl border border-border bg-[hsl(var(--bg-2))] p-4"
            id="case-types"
          >
                  ...
                  formatter={(value, entry) => (
                    <span
                      ...
                      style={{
                        ...
                        opacity:
                          entry?.dataKey && isSeriesActive(entry.dataKey as string) ? 1 : 0.4,
                        cursor: 'pointer',
                      }}
                    >
                      {value}
            ...
            <table className="w-full min-w-[640px] text-sm">
              ...
              <tbody>
                {outcomeStats.case_type_breakdown.map((item, index) => (
                  <tr
                    key={item.case_type}
                    className={index % 2 === 0 ? 'bg-[hsl(var(--bg-1))]/40' : ''}
                  >
                    <td className="py-2 text-left text-[color:hsl(var(--text-1))]">
                      {item.case_type}
                    </td>
                    <td className="py-2 text-right text-[color:hsl(var(--text-2))]">
                      {item.total_cases}
                    </td>
                    <td className="py-2 text-right text-[color:hsl(var(--pos))]">
                      {(item.win_rate * 100).toFixed(1)}%
                    </td>
                    <td className="py-2 text-right text-[color:hsl(var(--accent))]">
                      {(item.settlement_rate * 100).toFixed(1)}%
                    </td>
                    <td className="py-2 text-right text-[color:hsl(var(--warn))]">
