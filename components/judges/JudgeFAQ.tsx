...
export function JudgeFAQ({ judgeName }: JudgeFAQProps): JSX.Element {
    ...
    <section className="overflow-hidden rounded-2xl border border-border bg-[hsl(var(--bg-2))] shadow-md">
      ...
      <div className="space-y-3 px-6 py-5 text-sm text-[color:hsl(var(--text-2))]">
        {faqs.map((faq, index) => (
          <article key={faq.question} className="border-b border-border/60 pb-3 last:border-0">
            ...
            >
              <span className="font-medium text-[color:hsl(var(--text-1))] break-words">
                {faq.question}
              </span>
              {openIndex === index ? (
      ...
      <footer className="border-t border-border/60 bg-[hsl(var(--bg-1))] px-6 py-4 text-sm text-[color:hsl(var(--text-2))]">
        <p className="leading-relaxed">
          <strong className="text-[color:hsl(var(--text-1))]">Need more information?</strong>{' '}
          Contact the court clerk&apos;s office or consult with an attorney experienced in
          practicing before {judgeName}.
        </p>
      </footer>
    </section>
  )
