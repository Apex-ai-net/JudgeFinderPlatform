...
async function getPerformanceData() {
  ...
  const criticalEndpoints = ['execute_search', 'generate_bias_analytics', 'load_profile']
...
export default async function PerformancePage(): Promise<JSX.Element> {
  ...
  const { summary, endpointStats, cacheStats, recentErrors, slowQueries } =
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm font-semibold text-gray-900">
                          {Math.round(snapshot.p50)}ms
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-xs text-gray-500">P95</span>
                        <span
                          className={`text-sm font-semibold ${snapshot.p95 > 1000 ? 'text-orange-600' : 'text-gray-900'}`}
                        >
                      ...
                      <div className="flex justify-between">
                        ...
                        <span
                          className={`text-sm font-semibold ${snapshot.p99 > 2000 ? 'text-red-600' : 'text-gray-900'}`}
                        >
                      ...
                      <div className="flex justify-between">
                        ...
                        <span
                          className={`text-sm font-semibold ${snapshot.error_rate > 0.01 ? 'text-red-600' : 'text-green-600'}`}
                        >
            ...
            <div className="bg-white shadow-sm border border-gray-200 rounded-lg p-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  ...
                  <div
                    className={`text-2xl font-bold ${cacheStats.hit_rate > 0.7 ? 'text-green-600' : 'text-orange-600'}`}
                  >
                ...
                <div>
                  ...
                  <div className="text-2xl font-bold text-gray-900">
                    {cacheStats.avg_latency_ms.toFixed(1)}ms
                  </div>
                </div>
              </div>
            </div>
                  ...
                  {summary.length === 0 ? (
                    ...
                    summary.map((metric: PerformanceMetric, index: number) => (
                      <tr key={index} className="hover:bg-gray-50">
                        ...
                        <td
                          className={`px-6 py-4 whitespace-nowrap text-right text-sm font-medium ${metric.p95_duration_ms > 1000 ? 'text-orange-600' : 'text-gray-900'}`}
                        >
                          {Math.round(metric.p95_duration_ms)}
                        </td>
                        <td
                          className={`px-6 py-4 whitespace-nowrap text-right text-sm font-medium ${metric.p99_duration_ms > 2000 ? 'text-red-600' : 'text-gray-900'}`}
                        >
                          {Math.round(metric.p99_duration_ms)}
                        </td>
                        <td
                          className={`px-6 py-4 whitespace-nowrap text-right text-sm font-medium ${metric.error_rate_percent > 1 ? 'text-red-600' : 'text-green-600'}`}
                        >
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {slowQueries.map((query: any, index: number) => (
                      <tr key={index} className="hover:bg-gray-50">
                        <td className="px-6 py-4 text-sm text-gray-900">{query.operation}</td>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {recentErrors.map((error: any, index: number) => (
                      <tr key={index} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900">{error.operation}</td>
