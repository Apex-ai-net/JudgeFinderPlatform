...
export default async function CityPage({
  ...
  const courts = (data || []).filter((row) =>
    cityMatches(row.name as string, row.address as any, cityName)
  )
  ...
  return (
    <div className="min-h-screen bg-background text-foreground px-4 py-12">
      <div className="mx-auto max-w-7xl">
        ...
        {error && <div className="text-destructive mb-4">Failed to load courts</div>}
        {courts.length === 0 ? (
