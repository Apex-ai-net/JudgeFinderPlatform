...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!limitInfo.allowed) {
      ...
      if (typeof limitInfo.remaining === 'number')
        r.headers.set('RateLimit-Remaining', String(limitInfo.remaining))
    ...
    if (format === 'json') {
      ...
      if (typeof limitInfo.remaining === 'number')
        res.headers.set('RateLimit-Remaining', String(limitInfo.remaining))
      if (limitInfo.reset) res.headers.set('RateLimit-Reset', String(limitInfo.reset))
      return res
    }

    const header =
      'judge_id,full_name,court_id,court_name,jurisdiction,slug,courtlistener_id,source,last_updated'
    const rows = [header]
    for (const j of judges || []) {
      const source = j.courtlistener_id
        ? `CourtListener:https://www.courtlistener.com/person/${j.courtlistener_id}`
        : ''
      const csvRow = [
        j.id,
