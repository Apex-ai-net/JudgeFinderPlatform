  ...
  try {
    ...
    if (!limit.allowed) {
      ...
      if (typeof limit.remaining === 'number')
        r429.headers.set('RateLimit-Remaining', String(limit.remaining))
    ...
    for (const r of normalized) {
      ...
      const bucket =
        type ||
        (text.includes('summary judgment')
          ? 'summary_judgment'
      ...
      if (text.includes('granted') || text.includes('grant in part'))
        motionBuckets[bucket].granted++
    }
