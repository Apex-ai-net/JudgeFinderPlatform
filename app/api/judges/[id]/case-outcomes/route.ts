...
function calculateCaseTypeBreakdown(cases: any[]) {
  ...
  return Object.entries(caseTypeGroups)
    .map(([caseType, casesInType]) => {
      ...
      const outcomes = typedCases.reduce(
        (acc: Record<string, number>, case_: any) => {
          const outcome = normalizeOutcome(case_.outcome || case_.status)
          acc[outcome] = (acc[outcome] || 0) + 1
          return acc
        },
      ...
      const avgDuration =
        casesWithDuration.length > 0
          ? casesWithDuration.reduce((sum: number, duration: number) => sum + duration, 0) /
            casesWithDuration.length
          : 0
...
function calculateYearlyTrends(cases: any[]) {
  ...
  return Object.entries(yearlyGroups)
    .map(([year, yearCases]) => {
      ...
      const outcomes = typedYearCases.reduce(
        (acc: Record<string, number>, case_: any) => {
          const outcome = normalizeOutcome(case_.outcome || case_.status)
          acc[outcome] = (acc[outcome] || 0) + 1
          return acc
...
function calculatePerformanceMetrics(cases: any[], caseTypeBreakdown: any[]) {
  ...
  const efficiencyScore =
    monthlyAverages.length > 0
      ? monthlyAverages.reduce((sum: number, count: number) => sum + count, 0) /
...
function normalizeOutcome(outcome: string): string {
  ...
  if (
    outcomeStr.includes('judgment') ||
    outcomeStr.includes('ruling') ||
    outcomeStr.includes('verdict')
  )
    return 'judgment'
