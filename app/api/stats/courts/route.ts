...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const typeBreakdown =
      courtTypes?.reduce((acc: Record<string, number>, court) => {
    ...
    const totalJudgeCount =
      judgeData?.reduce((sum, court) => sum + (court.judge_count || 0), 0) || 0
    const courtsWithJudges =
      judgeData?.filter((court) => typeof court.judge_count === 'number' && court.judge_count > 0)
        .length || 0
    const avgJudgesPerCourt =
      courtsWithJudges > 0 ? Math.round(totalJudgeCount / courtsWithJudges) : null

    // Format court types for display
    const mainCourtTypes = {
      state: (typeBreakdown['state'] || 0) + (typeBreakdown['State'] || 0),
      local:
        (typeBreakdown['local'] || 0) +
        (typeBreakdown['Local'] || 0) +
        (typeBreakdown['municipal'] || 0) +
        (typeBreakdown['Municipal'] || 0),
