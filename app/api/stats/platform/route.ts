...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const [
      ...
      { data: monthActivity, error: activityError },
      { data: oldestCase, error: caseError },
    ] = await Promise.all([
    ...
    const searchesLast30 = (monthActivity || []).filter(
      (item) => item.activity_type === 'search'
    ).length
    ...
    if (oldestCase && oldestCase[0]?.date_filed) {
      ...
      const diffYears = Math.floor(
        (currentDate.getTime() - oldestDate.getTime()) / (1000 * 60 * 60 * 24 * 365)
      )
    ...
    const stats = {
      ...
      yearsOfData,
      yearsOfDataDisplay: yearsOfData
        ? `${yearsOfData} ${yearsOfData === 1 ? 'Year' : 'Years'} Historical Data`
        : '—',
