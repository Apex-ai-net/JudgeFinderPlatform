...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const analyticsCoverage =
      totalJudges && judgesWithAnalytics
    ...
    if (appointmentData && appointmentData.length > 0) {
      ...
      const experiences = appointmentData
        .map((judge) => {
    ...
    const lastUpdateIso =
      lastUpdated && lastUpdated[0]?.updated_at
        ? new Date(lastUpdated[0].updated_at).toISOString()
        : null
    ...
    const stats = {
      ...
      avgExperienceDisplay:
        typeof avgExperience === 'number' ? `${avgExperience} Years Experience` : '—',
